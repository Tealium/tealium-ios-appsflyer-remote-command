...
@objc
public class AppsFlyerRemoteCommand: NSObject {
    ...
    public func remoteCommand() -> TEALRemoteCommandResponseBlock {
        return { response in
            guard var payload = response?.requestPayload as? [String: Any] else {
                return
            }
    ...
    public func parseCommands(_ commands: [String], payload: [String: Any]) {
        commands.forEach { [weak self] command in
            ...
            case .initialize:
                ...
                return self.appsFlyerCommandTracker.initialize(appId: appId, appDevKey: appDevKey, config: settings)
            case .launch:
